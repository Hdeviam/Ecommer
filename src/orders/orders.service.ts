...
export class OrdersService {
  ...
  async checkExistingOrder(userId: string, quantity: number): Promise<boolean> {
    const existingOrders = await this.ordersRepository.getOrdersByUserId(userId);
    return existingOrders.some(order => 
      order.orderDetails.some(detail => detail.quantity === quantity)
    );
  }
  ...
  async addOrder(userId: string, productsData: { id: string; quantity: number }[]): Promise<Order> {
    ...
    for (const productData of productsData) {
      ...
      // Verificar si ya existe una orden con la misma cantidad
      const exists = await this.checkExistingOrder(userId, productData.quantity);
      if (exists) {
        throw new BadRequestException(`Ya has creado una orden con la cantidad ${productData.quantity}.`);
      }
    }
